...
-(void)showWinner
{
    ...
    BOOL computerWon;
    Judge *gameJudge = [[Judge alloc] init];
    
    NSString *computer = [[NSString alloc] init];
    
    computer = computerHandLabel.text;
    
    NSString *person = [[NSString alloc] init];
    
    person = playerHandLabel.text;
    
    computerWon = [gameJudge didComputerWin:person computer:computer] ;
...
-(void)countDown
{
    ...
    if (count == 0) {
        [timer invalidate];
        count = 4;
        ComputerHand *computersPick = [[ComputerHand alloc] init];
        computerHandLabel.text = [computersPick computerChoice];
